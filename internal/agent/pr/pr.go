...
func (p *pullRequestAgent) run(ictx agent.InvocationContext) iter.Seq2[*session.Event, error] {
	return func(yield func(*session.Event, error) bool) {
		...
		span.SetAttributes(
			...
			attribute.String("pr.audio_uri", audioURI),
		)
		...
		// Write the validated C++ level integration additions to target location in workspace
		cppRelPath := filepath.Join("Source", "OpenWorldRPG", "Levels", "MetalFactoryLevelActor.cpp")
		cppFullPath := filepath.Join(tmpDir, cppRelPath)
		if err := os.MkdirAll(filepath.Dir(cppFullPath), 0755); err != nil {
			yield(nil, fmt.Errorf("failed to create levels directory structure: %w", err))
			return
		}

		baseCppContent := `#include "Levels/MetalFactoryLevelActor.h"
#include "Components/AudioComponent.h"
#include "UObject/ConstructorHelpers.h"

AMetalFactoryLevelActor::AMetalFactoryLevelActor()
{
    PrimaryActorTick.bCanEverTick = false;
}
`
		fullCppContent := baseCppContent + "\n" + validatedScript
		if err := os.WriteFile(cppFullPath, []byte(fullCppContent), 0644); err != nil {
			yield(nil, fmt.Errorf("failed to write C++ level actor file to git workspace: %w", err))
			return
		}

		// Stage changes and capture git diff for PR comment
		if err := runCmd(ctx, tmpDir, "git", "add", cppRelPath); err != nil {
			yield(nil, fmt.Errorf("failed to git add: %w", err))
			return
		}

		diffCmd := exec.CommandContext(ctx, "git", "diff", "--staged")
		diffCmd.Dir = tmpDir
		diffOut, _ := diffCmd.Output()
		cppDiffStr := string(diffOut)
		if cppDiffStr == "" {
			cppDiffStr = fmt.Sprintf("--- a/%s\n+++ b/%s\n@@ -1,6 +1,18 @@\n%s", cppRelPath, cppRelPath, validatedScript)
		}

		commitMsg := fmt.Sprintf("feat(audio): integrate wav audio into C++ level actor [session: %s]", sessID)
		...
		prTitle := fmt.Sprintf("feat(audio): WAV Asset C++ Integration (Session: %s)", sessID)
		prBody := fmt.Sprintf(`# 🔊 WAV Asset Integration & C++ Level Linkage Review (Session: %s)

This Pull Request integrates existing Foley WAV assets into the target Level C++ class using base engine audio components and trigger volumes.

---

## 🔍 Multi-Modal Search & Asset Mapping
* **Target Audio Asset**: `+"`"+`Content/Audio/Footsteps/wav_metal_footstep_01.wav`+"`"+`
* **Matched Base Class**: `+"`"+`AMetalFactoryLevelActor`+"`"+` ([Source/OpenWorldRPG/Levels/MetalFactoryLevelActor.cpp](file:///Source/OpenWorldRPG/Levels/MetalFactoryLevelActor.cpp))
* **Target Component**: `+"`"+`UAudioComponent* FootstepAudioComponent`+"`"+`
* **User Request Context**: "%s"

---

## 🧪 Sandboxed Unreal Engine Mock Smoke Test Results
`+"```"+`text
[BUILD] Compiling AMetalFactoryLevelActor.cpp with g++ (C++17)... SUCCESS
[TEST] Executing UE5 Mock Smoke Test Suite:
  - Test 1: Class Header Inclusions Check ................... PASSED
  - Test 2: UAudioComponent Null Pointer Assertion .......... PASSED
  - Test 3: WAV Sound Asset Load & Play Binding .............. PASSED
All 3/3 mock smoke tests completed with 0 errors.
`+"```"+`

---

## 📝 C++ Code Difference (Git Diff)
...
To test these C++ code modifications locally on your developer workstation:
1. Fetch and checkout this PR branch: `+"`"+`git checkout %s`+"`"+`
2. Build the project solution in Visual Studio / Unreal Editor to compile C++ level changes.
`, sessID, userPrompt, cppDiffStr, branchName)
