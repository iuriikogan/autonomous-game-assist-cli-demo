...
func TestUnrealAgent_Instantiation(t *testing.T) {
	mockModel := &mockLLM{NameFunc: "gemini-3.1-pro"}
	mockVecTool := &mockTool{NameFunc: "vector_search", DescriptionFunc: "Mock vector search"}

	agent, err := New(mockModel, mockVecTool)
	if err != nil {
		t.Fatalf("failed to instantiate Unreal Agent: %v", err)
	}

	if agent.Name() != "unreal_agent" {
		t.Errorf("expected agent name 'unreal_agent', got %q", agent.Name())
	}
